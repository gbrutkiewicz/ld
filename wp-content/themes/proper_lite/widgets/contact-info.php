...
class properlite_Contact_Info extends WP_Widget { 



// constructor

    function properlite_contact_info() {

		$widget_ops = array('classname' => 'properlite_contact_info_widget', 'description' => __( 'Display your contact info', 'properlite') ); 

        parent::__construct(false, $name = __('MT - Contact Info Widget', 'properlite'), $widget_ops);

		$this->alt_option_name = 'properlite_contact_info';
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'address' )); ?>"><?php _e( 'Enter your address', 'properlite' ); ?></label>
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'phone' )); ?>"><?php _e( 'Enter your phone number', 'properlite' ); ?></label>
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'email' )); ?>"><?php _e( 'Enter your email address', 'properlite' ); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['properlite_contact_info']) )

			delete_option('properlite_contact_info');		  
	...
	function flush_widget_cache() {
		...
		wp_cache_delete('properlite_contact_info', 'widget');
	...
	function widget($args, $instance) {
		...
		if ( ! $this->is_preview() ) {
			...
			$cache = wp_cache_get( 'properlite_contact_info', 'widget' );
		...
		$title = ( ! empty( $instance['title'] ) ) ? esc_attr( $instance['title'] ) : __( 'Contact info', 'properlite' );
		...
		if( ($address) ) {
			...
			echo '<span>' . __('Address: ', 'properlite') . '</span>' . wp_kses_post( $address );
		...
		if( ($phone) ) {
			...
			echo '<span>' . __('Phone: ', 'properlite') . '</span>' . wp_kses_post( $phone );
		...
		if( ($email) ) {
			...
			echo '<span>' . __('Email: ', 'properlite') . '</span>' . '<a href="mailto:' . sanitize_email( $email ) . '">' . sanitize_email( $email ) . '</a>';
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'properlite_contact_info', $cache, 'widget' ); 
