...
class properlite_home_news extends WP_Widget {



// constructor

    function properlite_home_news() {

		$widget_ops = array('classname' => 'properlite_home_news_widget', 'description' => __( 'Show your blog posts on your home page.', 'properlite') );

        parent::__construct(false, $name = __('MT - Home Posts', 'properlite'), $widget_ops);

		$this->alt_option_name = 'properlite_home_news_widget'; 
	...
	// Check values
		...
		$read_more_text  = isset( $instance['read_more_text'] ) ? esc_html( $instance['read_more_text'] ) : __( 'View More', 'properlite' );
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['properlite_home_news']) )

			delete_option('properlite_home_news');	  
	...
	function flush_widget_cache() {
		...
		wp_cache_delete('properlite_home_news', 'widget');
	...
	function widget($args, $instance) {
		...
		if ( ! $this->is_preview() ) {
			...
			$cache = wp_cache_get( 'properlite_home_news', 'widget' );
...
?>
                        	...
                        	<div class="news-content">
                                ...
                                <span>
									...
									<?php $image = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_id() ), 'full' );
                                          $image = $image[0]; ?>
                                          <div class="news-image" style="background-image: url('<?php echo esc_url( $image ); ?>');">pokpok</div>
                                    <?php endif; ?> 
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'properlite_home_news', $cache, 'widget' );
